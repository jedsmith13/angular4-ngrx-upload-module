...
@Component({
  selector: 'ngrx-upload-body',
  template: `
        <div class="dropzone" ngrxUploadDragAndDrop
             (onDropFile)="onDropFile($event)"
             (onFileOverZone)="onFileOverZone($event)"
             (onFileLeaveZone)="onFileLeaveZone($event)">
        ...
        <ngrx-upload-file-form (onChangeFilesFromInput)="onChangeFilesFromInput($event)"></ngrx-upload-file-form>
        </div>
    `,
  styleUrls: ['./upload-body.component.css'],
  changeDetection: ChangeDetectionStrategy.OnPush
})
export class UploadBodyComponent {
  /**
   *
   ...
   */
  @Input()
  currentStoreFileCount: File;

  @Input()
  id: number;

  constructor(
    private _dragNdrop: DragAndDropService,
  ...
  onFileOverZone(event: Event) {
    // console.log(
    //   `[${CLASS}] Event on file over zone been called is file over zone? => `,
    //   this.isFileOverZone
    // );

    if (this.isFileOverZone) {
      // console.log(
      //   `[${CLASS}] No need to modify the state that file over the zone.`
      // );
      return;
    }
  ...
  onDropFile(fileList: FileList) {
    const files = this.createFilePhotoModelFromFileList(fileList);
  ...
  private fileCountLimitValidation(count) {
    ...
    return files => {
      if (count + files.length > 10) {
        // console.error(
        //   `[${CLASS}] You reach the maximum images you can upload. `
        // );
        alert('You can upload maximum 10 photos at once.');
        return;
      }

      this._fileHandler.addFile(this.id, files);
    };
  }
   ...
   * @memberof UploadBodyComponent
   */
  private createFilePhotoModelFromFileList(
    fileList: FileList
  ): FilePhotoModel[] {
    const immutateFileList = Array.from(fileList).concat([]);
    let filePhotoModels: FilePhotoModel[] = [];
